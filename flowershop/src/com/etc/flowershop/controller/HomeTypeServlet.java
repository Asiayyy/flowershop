...
@WebServlet("/HomeTypeServlet")
public class HomeTypeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            flowers = tuserService.hometype();
            //因为页面中需要得到flower所以需要进行存储
            request.setAttribute("flowerlist", flowers);
            request.getRequestDispatcher("hometype.jsp").forward(request, response);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
